...
struct FontOsdInfo_s {
	struct FontInfo_s font;						 /* 31 */
	struct FontWinInfo_s win[FOSD_MAX_OSDWIN];	 /* 9*8 */
} FontOsdInfo;
...
void DumpFontInfo(void)
{
	...
	wPrintf(" %bdx%bd", FontOsdInfo.font.w,FontOsdInfo.font.h);
	wPrintf(" bpp2:%x",FontOsdInfo.font.bpp2);
	wPrintf(" bpp3:%x",FontOsdInfo.font.bpp3);
	wPrintf(" bpp4:%x",FontOsdInfo.font.bpp4);
	wPrintf(" end:%x",FontOsdInfo.font.max);
}
...
void FOsdWinAlphaPixel(BYTE winno, BYTE lut, BYTE alpha)
{
	...
	WriteTW88(REG_FOSD_ALPHA_SEL,  lut );	// first, select color index
...
void FOsdDownloadFontByDMA(WORD dest_loc, DWORD src_loc, WORD size)
{
	...
	//SpiFlashDmaRead(DMA_DEST_FONTRAM, dest_loc, src_loc, size, SPIFLASH_WAIT_READ);
	SpiFlash_Read_FOsd(src_loc, dest_loc, size); 
...
void FOsdIndexMsgPrint(BYTE index)
{
	...
	case FOSD_STR0_GOOD:	
		...
		break;
	case FOSD_STR1_TW8836:
		TWstrcpy(FOsdMsgBuff,"Intersil TW8836.");
		break;
