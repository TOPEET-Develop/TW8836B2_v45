...
{
	Puts("\n\r  === Help for ChipTest command ===");
	Puts("\n\rCTest reg rw		;register read/write test");
	Puts("\n\rCTest reg default	;register default value");
	Puts("\n\rCTest gpio			;gpio test");
	Puts("\n\rCTest sosdlut		;SpiOSD LUT readback test");
	Puts("\n\rCTest dma [n_wait]	;spiflash dma wait test");
	Puts("\n\rCTest resume		;power suspend resume");
}
...
void ChipTest_Register_RW(BYTE page)
{
	...
	for(i=0; i < 16; i++ ) {
		for(j=0; j < 16; j++) {
			...
			if(pWMask[Index]) {
				Printf("\n\rREG%01bx%02bx WMask:%02bx RMask:%02bx Def:%02bx",page, Index, pWMask[Index], pRMask[Index], pDefData[Index]);
				...
				//read only.
				Printf("\n\rREG%01bx%02bx WMask:%02bx RMask:%02bx Def:__",page, Index, pWMask[Index], pRMask[Index]);
				...
				value = ReadTW88(PageIndex);
				if(value != 0xBF) {
					Printf("\n\rREG%01bx%02bx BF. Read:%02bx", page, Index, value);

					WriteTW88(PageIndex, 0xFF);
					value = ReadTW88(PageIndex);
					if(value != 00) {
						Printf("\n\rREG%01bx%02bx BF. but writable.", page, Index);
					}
				}
				else {
					//assume NO register.
...
void ChipTest_SOsdLut(BYTE winno, WORD LutOffset, WORD size,DWORD address)
{
	...
	SOsd_SetLut(winno, /*type=*/1, LutOffset, size*4, address, 0xFF);
	...
	for(ii=LutOffset; ii < (LutOffset+size); ii++) {
		...
		SpiFlash_Read_XMem(address+(ii-LutOffset)*4, (WORD)bgra, 4);
		//SpiFlashDmaRead2XMem(bgra, address+(ii-LutOffset)*4, 4, SPIFLASH_WAIT_READ);
		...
		for(j=0; j < 4; j++) {
			...
			if(B0 != bgra[j]) {
				error++;
				if(error <= 10)
					Printf("\n\rfail %d+%bd R:%bx Wrote:%bx", ii,j, B0, bgra[j]);
			}
		}
	}
	if(error==0)
...
*/
	...
	if(value & 0x01) {
		...
		Printf("\n\rREG4C4:%02bx wait:%bd",value, i);
	}
	else
		Printf("\n\rREG4C4:%02bx",value);

	Printf("\n\rRead ID: %02bx %02bx %02bx",ReadTW88(REG4D0),ReadTW88(REG4D1),ReadTW88(REG4D2));
...
//		SystemPowerResume(1);
	...
	//-------<<SPIOSD LUT test>>-----------------------------
		...
		if(argc < 6) {
			Printf("\n\rUsage:CTest sosdlut winno LutOffset size address");
			return;
		}
		winno = a2h(argv[2]);
