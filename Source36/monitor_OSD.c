...
void MonOsdHelp(void)
{
	Puts("\n\r===== Monitor OSD Help =====\n");
	Puts("\n\rInit           : osd 0");
	Puts("\n\rBlock Fill(8)  : osd 1 dx dy w h cl");
	Puts("\n\rBlock Fill(16) : osd 16 dx dy w h cl");
	Puts("\n\rBlock Transfer : osd 2 sx sy dx dy w h");
	Puts("\n\rLUT Test(8)    : osd lut w cnt sx sy");
	Puts("\n\rLUT Test(16)   : osd lut16");
	Puts("\n\rChange attr all: osd attr winno val");
	Puts("\n\rFord Logo      : osd ford sx sy");
	Puts("\n\rFord Logo RGB  : osd RGB sx sy");
	Puts("\n\rFord Logo aRGB : osd aRGB sx sy");
	Puts("\n\rFord Logo bRGB : osd bRGB sx sy");
	Puts("\n\r16bit expansion: osd EXP16 sx sy");
	Puts("\n\r             --> All numbers are decimal");

	Puts("\n\rAnimation      : osd ani 1/0");
	Puts("\n\rDisp Pigeon    : osd pigeon");
	Puts("\n\rDisp Pigeon Linear  : osd pigeonl");
	Puts("\n\rRose Test      : osd rose");
	Puts("\n\r16bit YUV      : osd Harry");
	Puts("\n\rSpeedometer    : osd sm nn");
	Puts("\n\rSM init        : osd sm init");
	Puts("\n\rSM Auto        : osd sm auto");
	Puts("\n\rSM Jump Step   : osd sm jump nn");
	Puts("\n\rSM Go speed    : osd sm go nn");

	Puts("\n\r************* Issues **************");
	Puts("\n\r 1. Over 0xC000 is unstable?");
	Puts("\n\r 2. Busy control from OSD?");
	Puts("\n\r\n");
}
...
void MonOsdChangeAttr(BYTE winno, BYTE attr)
{
	...
	Printf("\n\rOSD %bd LUT set to all %02bx", winno, attr);
...
void MonitorOSD(BYTE wbits)
{
	...
	//---------- Block Fill -------------------
		...
		Printf("\n\rOsdBlockFill(%04x %04x %04x %04x ", dat[0], dat[1], dat[2], dat[3]);
		Printf("%04x)", dat[4]);
	...
	//---------- Block Transfer ----------------
		...
		Printf("\n\rOsdBlockTransfer (%04x %04x ", dat[0], dat[1]);
...
//		OsdWinBuffer( 4, ddraddr, Harry_Header->w/64, Harry_Header->h/64 + 1 );
		...
		Printf("\n\rPress ESC to display 8bit to 16bit expansion with transparent");
		while(1) {
				if( RS_ready() ) {
					ch = RS_rx();
					if ( ch == 27 ) break;
...
//			AnimationPigeon();
	...
	//---------- LUT Test-----------------------
		...
		Printf("\n\rChange Attribute in LUT: Window %d = %02x", dat[0], dat[1]);
		MonOsdChangeAttr(dat[0], dat[1]);
	}
	//---------- LUT Test-----------------------
	else if( !stricmp(argv[1], "LUTx") ) {
		Printf("\n\rset Transparent LUT: Window %d, R:%d, G:%d, B:%d", dat[0], dat[1], dat[2], dat[3]);
